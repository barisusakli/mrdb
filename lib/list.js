...
module.exports = function(db, module) {
	...
	module.lpush = function(key, value, callback) {
		...
		db.collection('list').findOne({k: key}, {array: 0}, function(err, item) {
			if (err) {
				return callback(err);
			}
			...
			if (item && item.k) {
				...
				for(var i=0; i<value.length; ++i) {
					bulk.find({k: key}).upsert().updateOne({$push: {array: {$each: [value[i]], $position: 0}}});
				}
	...
	module.rpush = function(key, value, callback) {
		...
		value = value.map(helpers.valueToString);
		db.collection('list').update({k: key}, {$push: {array: {$each: value}}}, {upsert: true, w: 1}, function(err) {
			callback(err);
		});
	};
	...
	module.rpop = function(key, callback) {
		...
		module.lrange(key, -1, -1, function(err, value) {
			...
			db.collection('list').update({k: key }, { $pop: { array: 1 } }, function(err) {
	...
	module.ltrim = function(key, start, stop, callback) {
		...
		module.lrange(key, start, stop, function(err, value) {
			...
			db.collection('objects').update({k: key}, {$set: {array: value}}, function(err) {
				callback(err);
			});
		});
	};
	...
	module.lrange = function(key, start, stop, callback) {
		...
		db.collection('list').findOne({k: key}, {array: 1}, function(err, data) {
