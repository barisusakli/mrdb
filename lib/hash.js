...
module.exports = function(db, module) {
	...
	module.hmset = function(key, data, callback) {
		...
		db.collection('hash').update({k: key}, {$set: data}, {upsert: true, w: 1}, function(err) {
			callback(err);
		});
	};
	...
	module.hgetall = function(keys, callback) {
		...
		if (Array.isArray(keys)) {
			...
			db.collection('hash').find({k: {$in: keys}}).toArray(function(err, data) {
				if (err) {
					return callback(err);
				}
			...
			db.collection('hash').findOne({k: keys}, {_id: 0}, function(err, data) {
				if (err) {
					return callback(err);
				}
				if (data) {
	...
	module.hget = function(key, field, callback) {
		...
		_fields[field] = 1;
		db.collection('hash').findOne({k: key}, {fields: _fields}, function(err, item) {
			if (err || !item) {
	...
	module.hmget = function(keys, fields, callback) {
		...
		if (Array.isArray(keys)) {
			db.collection('hash').find({k: {$in: keys}}, {fields: _fields}).toArray(function(err, items) {
				if (err) {
					return callback(err);
				}
			...
			db.collection('hash').findOne({k: keys}, {fields: _fields}, function(err, item) {
				if (err) {
					return callback(err);
				}
				item = item || {};
	...
	module.hexists = function(key, fields, callback) {
		...
		if (Array.isArray(fields)) {
			...
			db.collection('hash').findOne({k: key}, {fields: data}, function(err, item) {
				if (err) {
					return callback(err);
				}
				var results = [];
	...
	module.hdel = function(key, fields, callback) {
		...
		db.collection('hash').update({k: key}, {$unset : data}, function(err) {
			callback(err);
		});
	};
	...
	module.hincrby = function(key, field, value, callback) {
		...
		db.collection('hash').findAndModify({k: key}, {}, {$inc: data}, {new: true, upsert: true}, function(err, result) {
