...
module.exports = function(db, module) {
	...
	module.zscore = function(key, value, callback) {
		...
		if (Array.isArray(key)) {
			return module.sortedSetsScore(key, value, callback);
		}

		if (Array.isArray(value)) {
			return module.sortedSetScores(key, value, callback);
		}
	...
	module.sortedSetsScore = function(keys, value, callback) {
		...
		value = helpers.valueToString(value);
		db.collection('zset').find({k: {$in: keys}, v: value}, {_id: 0, k: 1, s: 1}).toArray(function(err, result) {
			if (err) {
				return callback(err);
			}
			...
			for(var i=0; i<keys.length; ++i) {
				item = map[keys[i]];
				returnData.push(item ? item.s : null);
			}
	...
	module.sortedSetScores = function(key, values, callback) {
		...
		values = values.map(helpers.valueToString);
		db.collection('zset').find({k: key, v: {$in: values}}, {_id: 0, v: 1, s: 1}).toArray(function(err, result) {
			if (err) {
				return callback(err);
			}
	...
	module.zismember = function(key, value, callback) {
		...
		if (Array.isArray(key)) {
			return module.isMemberOfSortedSets(key, value, callback);
		}

		if (Array.isArray(value)) {
			return module.isSortedSetMembers(key, value, callback);
		}

		value = helpers.valueToString(value);
		db.collection('zset').findOne({k: key, v: value}, {_id: 0, v: 1}, function(err, result) {
			callback(err, !!result);
		});
	};
	...
	module.isSortedSetMembers = function(key, values, callback) {
		...
		values = values.map(helpers.valueToString);
		db.collection('zset').find({k: key, v: {$in: values}}, {fields: {_id: 0, v: 1}}).toArray(function(err, results) {
			if (err) {
				return callback(err);
			}
	...
	module.isMemberOfSortedSets = function(keys, value, callback) {
		...
		value = helpers.valueToString(value);
		db.collection('zset').find({k: {$in: keys}, v: value}, {fields: {_id: 0, k: 1, v: 1}}).toArray(function(err, results) {
			if (err) {
				return callback(err);
			}
	...
	module.getSortedSetsMembers = function(keys, callback) {
		...
		db.collection('zset').find({k: {$in: keys}}, {_id: 0, k: 1, v: 1}).toArray(function(err, data) {
			if (err) {
				return callback(err);
			}
	...
	module.zunion = function(sets, start, stop, callback) {
		getSortedSetUnion(sets, 1, start, stop, callback);
	};

	module.zrevunion = function(sets, start, stop, callback) {
		getSortedSetUnion(sets, -1, start, stop, callback);
	};
	...
	function getSortedSetUnion(sets, sort, start, stop, callback) {
		...
		var pipeline = [
			{ $match: { k: {$in: sets}} },
			{ $group: { _id: {v: '$v'}, totalScore: {$sum : "$s"}} },
		...
		pipeline.push({	$project: { _id: 0, v: '$_id.v' }});

		db.collection('zset').aggregate(pipeline, function(err, data) {
			if (err || !data) {
				return callback(err);
			}
	...
	module.zincrby = function(key, increment, value, callback) {
		callback = callback || helpers.noop;
		if (!key) {
			return callback();
		}
		...
		data.s = parseInt(increment, 10);

		db.collection('zset').findAndModify({k: key, v: value}, {}, {$inc: data}, {new: true, upsert: true}, function(err, result) {
			callback(err, result && result.value ? result.value.s : null);
		});
	};

	module.zrangebylex = function(key, min, max, start, count, callback) {
		var query = {k: key};
		if (min !== '-') {
			query.v = {$gte: min};
		}
		...
		db.collection('zset').find(query, {_id: 0, v: 1})
			.sort({v: 1})
			.skip(start)
