...
module.exports = function(db, module) {
	...
	module.sadd = function(key, value, callback) {
		...
		value.forEach(function(value) {
			bulk.find({_id: key + '_' + value}).upsert().updateOne({$set: {k: key, v: value}});
		});
	...
	module.sremove = function(key, value, callback) {
		...
		var ids = value.map(function(value) {
			value = helpers.valueToString(value);
			return key + '_' + value;
		});

		db.collection('set').remove({_id: {$in: ids}}, function(err) {
			callback(err);
		});
	};
	...
	module.isSetMembers = function(key, values, callback) {
		...
		db.collection('set').find({_id: {$in: ids}}, {fields: {_id:0, v: 1}}).toArray(function(err, results) {
			if (err) {
				return callback(err);
			}
	...
	module.isMemberOfSets = function(sets, value, callback) {
		...
		var ids = sets.map(function(set) {
			return set + '_' + value;
		});

		db.collection('set').find({_id: {$in: ids}}, {_id: 0, v: 0}).toArray(function(err, result) {
			if (err) {
				return callback(err);
			}
